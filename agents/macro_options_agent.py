...
이미 정형화된 수치이로 복합 상태기계로 LLM과 동등한 해석 능.

단 LLM이 필요한 이스 (현재 코드는 fallback sentinel로 표시):
- 서술형 이벤트 deribit_context에 섞인 경우 (e.g. "Unusual block trade 50K contracts @ IV120")
- 여러 표 서로 모순되는 극단적 레짐 (DVOL↑ + PCR↓ = 상반된 공포 신호)
  → 이 경우 requires_llm=True를 반환해 meta_agent 추 해석

신호 로직:

1. DVOL (Deribit Vol Index)  공포/탐욕 표
   < 40  : 동성 레짐 (complacency) → breakout risk HIGH (방향 불명)
   40-60 : 정상 레짐
   60-80 : 高동성 (방어적 헤징 중) → BEARISH 편향
   > 80  : 공황 (→ 역발상: 너무 높으면 바닥 근처일 수 있음)

2. PCR OI (Put/Call Ratio by Open Interest)
   < 0.5 : 콜 편중 (탐욕/상승 베팅 과열) → contrarian BEARISH
   0.5-0.8: 균형
   > 0.8 : 풋 편중 (헤징 과열) → 하락 위험 HIGH, 하만 too_crowded이면 반등 능
   > 1.2 : 극단적 헤징 → 역발상 BULLISH (세력이 이미 방어 완료)

3. IV Term Structure (Skew)
   "INVERTED" in context → 단기 IV > 장기 IV = 즉각적 공황 헤징 (BEARISH 확정)
   skew25d:
     > +5% : 콜 스큐 (상방 프리미엄) → 기이 upside를 사고 있음 = BULLISH
     < -5% : 풋 스큐 (하방 프리미엄) → 기이 downside를 헤징 = BEARISH
...
복합 레짐 분류:
  ...
  INSTITUTIONAL_BUY: PCR>1.1 + skew25d>3 (기이 콜 매수 중 = upside 기)
...
class MacroOptionsAgent:
    ...
    def analyze(
        ...
        result = {
            ...
            "pcr_z": None,
            "regime": "NORMAL",
            "requires_llm": False,   # True = meta_agent에게 추 해석 위임 신호
        ...
        option_score = 0.0
        macro_score = 0.0
        contradictions = 0  # 표 모순 카운트 → requires_llm 트리거
        ...
        try:
            ...
            if dvol is not None:
                if dvol_z is not None:
                    ...
                    if dvol > self.DVOL_HIGH:
                        signals.append(f"DVOL={dvol:.0f} (절 PANIC level)")
                    ...
                    if dvol > self.DVOL_HIGH:
                        ...
                        signals.append(f"DVOL={dvol:.0f} (complacencybreakout risk)")
                        option_score += 0.1
            ...
            if pcr is not None:
                ...
                if pcr < self.PCR_CALL_OVERLOADED:
                    signals.append(f"PCR={pcr:.2f}{pcr_z_note} (call overloadedcontrarian BEARISH)")
                    ...
                    signals.append(f"PCR={pcr:.2f}{pcr_z_note} (extreme put hedgecontrarian BULLISH)")
                    ...
                    signals.append(f"PCR={pcr:.2f}{pcr_z_note} (elevated put hedgingBEARISH bias)")
                    option_score += 0.20 if (pcr_z is not None and pcr_z >= 2.0) else 0.15

            # ----------------------- 25d Skew -----------------------
            if skew is not None:
                if skew > self.SKEW_CALL_PREMIUM:
                    signals.append(f"25d skew={skew:+.1f}% (call premiuminstitutional LONG demand)")
                    if result["options_bias"] != "BEARISH":
                        result["options_bias"] = "BULLISH"
                    option_score += 0.2
                elif skew < self.SKEW_PUT_PREMIUM:
                    signals.append(f"25d skew={skew:+.1f}% (put premiumdownside hedge active)")
                    if result["options_bias"] != "BULLISH":
                        result["options_bias"] = "BEARISH"
                    option_score += 0.2
        ...
        try:
            ...
            if dgs10_1w_chg is not None:
                if dgs10_1w_chg > 0.3:
                    signals.append(f"DGS10 surged {dgs10_1w_chg:+.2f}% (Rate shockBEARISH macro)")
                    macro_signal = "BEARISH"
                    macro_score += 0.30
                elif dgs10_1w_chg < -0.3:
                    signals.append(f"DGS10 fell {dgs10_1w_chg:+.2f}% (Yields droppingBULLISH macro)")
                    macro_signal = "BULLISH"
                    macro_score += 0.20

            if dgs10 is not None and macro_signal == "NEUTRAL":
                if dgs10 > self.DGS10_HEADWIND:
                    signals.append(f"DGS10={dgs10:.2f}% (risk headwindBEARISH macro)")
                    macro_signal = "BEARISH"
                    macro_score += 0.25
                elif dgs10 < self.DGS10_BENIGN:
                    signals.append(f"DGS10={dgs10:.2f}% (rate-friendlyBULLISH macro)")
            ...
            if dxy is not None:
                if dxy > self.DXY_STRONG:
                    signals.append(f"DXY={dxy:.1f} (strong USDBEARISH risk assets)")
                    macro_signal = "BEARISH"
                    macro_score += 0.20
                elif dxy < self.DXY_WEAK:
                    signals.append(f"DXY={dxy:.1f} (weak USDBULLISH risk assets)")
                    if macro_signal != "BEARISH":
                        macro_signal = "BULLISH"
                    macro_score += 0.15
